...
export const TestViewPage = () => {
  ...
  useEffect(() => {
    const fetchPublishedData = async () => {
      try {
        ...
        setError("Failed to load published test.");
      } finally {
        setIsLoading(false);
      }
    };
    if (id) fetchPublishedData();
  }, [id]);

  if (error) {
    return (
      <div className="max-w-4xl mx-auto space-y-6 mt-12 px-4">
  ...
  if (isSubmitted) {
    return (
      <div className="max-w-2xl mx-auto mt-12 bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm px-4">
        <div className="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-6">
          <CheckCircle2 size={40} className="text-green-500" />
        </div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Test Submitted Successfully! 🎉</h2>
        <p className="text-gray-500 mb-6">You have completed <strong>{testData?.name}</strong>.</p>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-24 mt-6 px-4">
      <PageHeader 
        ...
        title={`${testData?.name || 'Untitled Test'}`}
        action={
          <Button onClick={() => setIsSubmitted(true)} className="bg-green-600 hover:bg-green-700 text-white">
            Submit Test
          </Button>
        }
      />
